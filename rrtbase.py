...
class RRTMap:
    ...
    def drawpath(self,path):
        for node in path:
            pygame.draw.circle(self.map, self.Green, node, self.nodeRad+5,1)
            #pygame.time.delay(10)
            
        for i in range(1,len(path)):
            pygame.draw.line(self.map,self.Green,path[i],path[i-1],self.edgeThickness+2)
...
class RRTGraph:
    def __init__(self, start, goal, MapDimensions, obsdim, obsnum):
        ...
        self.start = start
        self.goal = goal
        self.goalFlag = False   # set true when tree reaches goal
        self.maph, self.mapw = MapDimensions
        self.x = []
        self.y = []
        self.parent = []
